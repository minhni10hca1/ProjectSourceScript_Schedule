...
function savehomecustomerdb() {
    var options = {
        ...
        host: "192.168.1.184",
        port: 8080,
        path: "/API_CARPO/save-home-calculator-before",
        method: "GET"
    };
    var authRequest = https.request(options, function (authResponse) {
        ...
        authResponse.on("end", function () {
            ...
            if (status == 1) {
                logger.info("Cập nhật Save Calculator Home  Customer before Thành công!!!");
            } else {
                logger.error("Cập nhật Save Calculator Home Customer before Thất bại!!!");
            }
        });
    });
    authRequest.end();
